...
# GitHub installation launcher for a fresh Raspberry Pi OS. It downloads the
# current package on every run; installer_addon.sh owns all hardware decisions.
...
if [[ "$missing" == true ]]; then
    if ! DEBIAN_FRONTEND=noninteractive dpkg --force-confold --configure -a; then
        ...
        DEBIAN_FRONTEND=noninteractive dpkg --force-confold --configure -a || \
            die "Die Debian-Pakete konnten nicht vollständig konfiguriert werden."
    fi
